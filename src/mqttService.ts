...
async function checkGeofences(location: any) {
  ...
  for (const fence of fences) {
    const distance = getDistanceKm(latNum, lonNum, fence.center_lat, fence.center_lng);
    ...
    if (!lastNotification) {
      await notificationService.createNotification(String(userId), title, message);
    }
  }
}
...
client.on('message', async (topic, message) => {
  ...
  try {
    ...
    await checkGeofences(location.toJSON());
